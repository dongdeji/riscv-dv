"""
convert emulator sim log to standard riscv instruction trace format
"""
...
"""
RE_RE    = re.compile(r"(?P<reg>[xf]\s{0,2}\d+) p\s{0,2}\d+ 0x(?P<val>[0-9a-f]{16})")
CORE_RE  = re.compile(r"(?P<pri>\d) 0x(?P<addr>008[0-9a-f]*) \(0x(?P<bin>[0-9a-f]{8})\) (?P<reg>[xf]\s{0,2}\d+) 0x(?P<val>[0-9a-f]+) (?P<instr>.*)")
CORE0_RE = re.compile(r"(?P<pri>\d) 0x(?P<addr>008[0-9a-f]*) \(0x(?P<bin>[0-9a-f]{8})\) (?P<instr>.*)")
CORE1_RE = re.compile(r"(?P<pri>\d) 0x(?P<addr>008[0-9a-f]*) \(0x(?P<bin>[0-9a-f]{8})\) (?P<reg>[xf]\s{0,2}\d+) p\s{0,2}\d+ 0xXXXXXXXXXXXXXXXX (?P<instr>.*)")
...
def process_instr(trace):
    if trace.instr == "jal":
        # Emulator jal format jal rd, -0xf -> jal rd, -15
...
def read_emulator_instr(match, full_trace):
    ...
    # Emulator's disassembler shows a relative jump as something like "j pc +
...
def read_emulator_trace(path, full_trace):
    """Read a Emulator simulation log at <path>, yielding executed instructions.
    ...
    Since Emulator has a strange trampoline that always runs at the start, we skip
    ...
    # is that we're in state TRAMPOLINE until we get to the end of Emulator's
    ...
    with open(path, 'r') as handle:
        for line in handle:
            ...
            if instr is None:
                ...
                instr = read_emulator_instr(instr_match, full_trace)
            ...
            # the --log-commits Emulator option)?
...
def process_emulator_sim_log(emulator_log, csv, full_trace=0):
    """Process EMULATOR simulation log.

    Extract instruction and affected register information from emulator simulation
    log and write the results to a CSV file at csv. Returns the number of
    instructions written.

    """
    logging.info("Processing emulator log : {}".format(emulator_log))
    instrs_in = 0
    instrs_out = 0
    ...
    with open(csv, "w") as csv_fd:
        ...
        for (entry, illegal) in read_emulator_trace(emulator_log, full_trace):
            instrs_in += 1
...
def main():
    ...
    parser.add_argument("--log", type=str, help="Input emulator simulation log")
    ...
    # Process emulator log
    process_emulator_sim_log(args.log, args.csv, args.full_trace)
